...
package com.example.productivityback.auth.controller;

import com.example.productivityback.auth.event.OnGenerateResetLinkEvent;
import com.example.productivityback.auth.event.OnRegenerateEmailVerificationEvent;
import com.example.productivityback.auth.event.OnUserAccountChangeEvent;
import com.example.productivityback.auth.event.OnUserRegistrationCompleteEvent;
import com.example.productivityback.auth.exception.*;
import com.example.productivityback.auth.model.CustomUserDetails;
import com.example.productivityback.auth.model.payload.*;
import com.example.productivityback.auth.model.token.EmailVerificationToken;
import com.example.productivityback.auth.model.token.RefreshToken;
import com.example.productivityback.auth.security.JwtTokenProvider;
import com.example.productivityback.auth.service.AuthService;
...
public class AuthController {
    ...
    public ResponseEntity<ApiResponse> checkEmailInUse(@ApiParam(value = "Email id to check against") @RequestParam("email") String email) {
    ...
    public ResponseEntity<ApiResponse> checkUsernameInUse(@ApiParam(value = "Username to check against") @RequestParam(
    ...
    public ResponseEntity<ApiResponse> registerUser(@ApiParam(value = "The RegistrationRequest payload") @Valid @RequestBody RegistrationRequest registrationRequest) throws NotFoundException {
    ...
    @ApiOperation(value = "Receive the reset link request and publish event to send mail containing the password " +
            "reset link")
    public ResponseEntity<ApiResponse> resetLink(@ApiParam(value = "The PasswordResetLinkRequest payload") @Valid @RequestBody PasswordResetLinkRequest passwordResetLinkRequest) {
    ...
    @ApiOperation(value = "Reset the password after verification and publish an event to send the acknowledgement " +
            "email")
    public ResponseEntity<ApiResponse> resetPassword(@ApiParam(value = "The PasswordResetRequest payload") @Valid @RequestBody PasswordResetRequest passwordResetRequest) {
    ...
    public ResponseEntity<ApiResponse> confirmRegistration(@ApiParam(value = "the token that was sent to the user email") @RequestParam("token") String token) {
    ...
    @ApiOperation(value = "Resend the email registration with an updated token expiry. Safe to " +
            ...
            "any attempts at generating new token from past (possibly archived/deleted)" +
            "tokens should fail and report an exception. ")
    public ResponseEntity<ApiResponse> resendRegistrationToken(@ApiParam(value = "the initial token that was sent to the user email after registration") @RequestParam("token") String existingToken) {
    ...
    public ResponseEntity<JwtAuthenticationResponse> refreshJwtToken(@ApiParam(value = "The TokenRefreshRequest payload") @Valid @RequestBody TokenRefreshRequest tokenRefreshRequest) {
